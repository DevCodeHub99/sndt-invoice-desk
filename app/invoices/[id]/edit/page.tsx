...
export default function EditInvoicePage() {
    ...
    // Pre-fill the form when invoice and products are available
    useEffect(() => {
        if (!invoice || products.length === 0) return;

        setClientId(invoice.clientId);
        setNotes(invoice.notes || '');
        setAdvancePayment(invoice.advancePayment ? String(invoice.advancePayment) : '');

        // Calculate due days from dueDate
        ...
        setLineItems(restoredLineItems.length > 0 ? restoredLineItems : [{
            id: uuidv4(),
            productId: '',
            comment: '',
            quantity: 1,
        ...
        // Convert manpower charges back
        if (invoice.manpowerCharges && invoice.manpowerCharges.length > 0) {
            const restoredManpower: ManpowerItem[] = invoice.manpowerCharges.map((item) => {
                // Try to find matching labor charge by name
            ...
            setManpowerItems(restoredManpower);
        } else {
            setManpowerItems([]);
        }

        setIsLoading(false);
    }, [invoice, products, laborCharges, currentUser?.businessDetails?.defaultTaxRate]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!clientId || invoiceItems.length === 0 || !invoice) return;
    ...
    if (isLoading) {
        return (
            <div>
                <div className="mb-6">
                    <Link href={`/invoices/${invoiceId}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-4">
                        <ArrowLeft className="w-4 h-4" />
                        Back to Invoice
                    </Link>
                    <PageHeader title={`Edit Invoice ${invoice.invoiceNumber}`} description="Loading invoice data..." />
                </div>
                <Card>
                    <CardContent>
                        <div className="flex items-center justify-center py-16">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                            <span className="ml-3 text-muted-foreground">Loading invoice data...</span>
                        </div>
                    </CardContent>
                </Card>
            </div>
