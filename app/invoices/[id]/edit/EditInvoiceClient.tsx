...
import { Trash2, ArrowLeft, Save, Plus } from 'lucide-react';
import Link from 'next/link';
import { v4 as uuidv4 } from 'uuid';
import { invoiceSchema } from '@/lib/schemas';
import { apiCall } from '@/lib/api-client';
import type { InvoiceItem, Invoice, Client, Product, Labor, User } from '@/lib/types';
...
export default function EditInvoiceClient({
    ...
    const getInitialManpowerItems = (): ManpowerItem[] => {
        if (invoice.manpowerCharges && invoice.manpowerCharges.length > 0) {
            return invoice.manpowerCharges.map((item) => {
                const labor = laborCharges.find(l => l.name === item.description);
                return {
                    id: uuidv4(),
                    laborId: labor?.id || '',
    ...
    const [manpowerItems, setManpowerItems] = useState<ManpowerItem[]>(getInitialManpowerItems);
    const [notes, setNotes] = useState(invoice.notes || '');
    const [dueInDays, setDueInDays] = useState(getInitialDueDays);
    const [advancePayment, setAdvancePayment] = useState<string>(invoice.advancePayment ? String(invoice.advancePayment) : '');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!clientId || invoiceItems.length === 0 || isSaving) return;
        ...
        try {
            ...
            const { ok } = await apiCall(`/api/invoices/${invoice.id}`, {
                ...
                body: JSON.stringify({
                    ...
                    status: totals.advancePayment > 0 ? (totals.balanceDue === 0 ? 'paid' : 'partial') : invoice.status,
                    notes,
                    dueDate,
                })
            });
    ...
    const canSubmit = clientId && invoiceItems.length > 0 && !isSaving;

    return (
        <div>
            <div className="mb-6">
                <Link href={`/invoices/${invoice.id}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-4">
                    <ArrowLeft className="w-4 h-4" />
                    Back to Invoice
                </Link>
                <PageHeader
                    title={`Edit Invoice ${invoice.invoiceNumber}`}
                    description="Modify the invoice details below. Invoice number and date remain unchanged."
                />
            </div>

            <div className="mb-6 inline-flex items-center gap-2 px-4 py-2 bg-muted/50 rounded-lg border">
                <span className="text-xs text-muted-foreground">Invoice #</span>
                <span className="text-sm font-bold text-foreground">{invoice.invoiceNumber}</span>
                <span className="text-xs text-muted-foreground">•</span>
                <span className="text-xs text-muted-foreground">
                    Created {new Date(invoice.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                </span>
            </div>
                        ...
                        <Card className="lg:sticky lg:top-8">
                            ...
                            <CardFooter className="flex-col gap-3">
                                ...
                                <Link
                                    ...
                                    className="inline-flex items-center justify-center font-medium rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 h-10 px-4 text-sm bg-secondary text-secondary-foreground hover:bg-secondary/80 w-full mt-2"
                                >
                                    Cancel
                                </Link>
                            </CardFooter>
